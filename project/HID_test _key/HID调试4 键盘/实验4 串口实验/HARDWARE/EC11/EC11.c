...
void EXTI9_5_IRQHandler(void)
{
    if(EXTI_GetITStatus(EC11_A_EXTI_Line)!= RESET)//A进入中断，下降沿触发。
    {
        ...
        if(encoder_direct_flag==2&&EC11_B_STATE == LOW) {//第一次和第二次转都是逆时针
...
void EXTI1_IRQHandler(void) {
    if(EXTI_GetITStatus(EC11_B_EXTI_Line)!= RESET) { //B进入中断，下降沿触发。
        if(EC11_A_STATE == HIGH)//B先于A触发下降沿，且A是高电平，第一次可能是顺时针
        {
            encoder_direct_flag=2;
        }
        if(encoder_direct_flag==1&&EC11_A_STATE == LOW)//第一次和第二次转都是逆时针
        {
    ...
    EXTI_ClearITPendingBit(EC11_B_EXTI_Line);    //EC11_B_EXTI_Line
