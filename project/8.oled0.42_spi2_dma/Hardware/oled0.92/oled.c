...
void DMA_OLED_Init(void){
  ...
  DMA_InitTypeDef DMA_InitStructure;
	...
	DMA_DeInit(OLED_042_DMA);
	
	DMA_InitStructure.DMA_PeripheralBaseAddr = (u32)(&(OLED_042_SPI->DR)); //SPI数据寄存器地址
	DMA_InitStructure.DMA_MemoryBaseAddr = (u32)OLED_GRAM; //设置接收缓冲区首地址
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralDST; //从储存器读取发送到外设
	DMA_InitStructure.DMA_BufferSize = 88; //DMA 通道的 DMA 缓存的大小
	...
	DMA_Init(OLED_042_DMA, &DMA_InitStructure); //根据指定的参数初始化
  //DMA_Cmd(DMA1_Channel3, DISABLE); //不使能DMA1 CH3所指示的通道
  DMA_Cmd(OLED_042_DMA, ENABLE); //使能DMA1 CH3所指示的通道
}
...
void OLED_WR_CMD(u8 dat) {  
  // 确保SPI传输空闲
    ...
    while (SPI_I2S_GetFlagStatus(OLED_042_SPI, SPI_I2S_FLAG_BSY) == SET){}//记得加‘；’	或者‘{}’	
		SPI_I2S_DMACmd(OLED_042_SPI, SPI_I2S_DMAReq_Tx, DISABLE);	
...
void OLED_WR_Multi_Bytes(uint8_t *data, uint16_t size) {
    ...
    DMA1_Channel5->CNDTR = size; // 设置传输数据的数量
    DMA1_Channel5->CMAR = (uint32_t)data; // 设置数据源地址
    DMA1_Channel5->CPAR = (uint32_t)(&(SPI2->DR)); // 设置SPI数据寄存器地址
